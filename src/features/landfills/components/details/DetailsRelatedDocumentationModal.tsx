...
export function DetailsRelatedDocumentationModal() {
  ...
  const { details } = useLandfillDetails(selectedLandfillId);

  const docs = details ? details.multimedia ? details.multimedia.filter((multi) => multi.hasCategory("PDF")) : [] : [];
      ...
      <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50/50 px-4 py-3">
        <div>
          <h2 className="text-sm font-semibold text-slate-800">
            {t("details.related_docs_modal.title")}
          </h2>
          {details && (
        ...
        {docs.length > 0 ? (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1 pt-0.5">
                    <h3 className="line-clamp-2 text-sm font-medium text-slate-700 transition-colors group-hover:text-emerald-800">
                      {doc.fileName || t("details.related_docs_modal.untitled")}
                    </h3>
                    <p className="mt-0.5 flex items-center gap-1 text-[11px] text-slate-400">
                      {/* TODO: Maybe in a future more related document types are added here */}
                      PDF
                      <span className="h-0.5 w-0.5 rounded-full bg-slate-300" />
                      <span>
                        {t("details.related_docs_modal.open_new_tab")}
                      </span>
                    </p>
                  </div>
          ...
          <div className="flex h-full flex-col items-center justify-center p-6 text-center text-slate-400">
            <p className="text-sm font-medium text-slate-600">
              {t("details.related_docs_modal.no_docs")}
            </p>
          </div>
        )}
      </div>
